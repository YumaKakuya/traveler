set_project("traveler")
set_version("0.1.0")
set_languages("c++20")

add_rules("mode.debug", "mode.release")

includes("third_party/")

add_requires("ftxui", "tl_expected", "nlohmann_json", "sqlite3", "tree-sitter")

-- Cloud provider support (cpp-httplib+openssl) is Linux-only for now.
-- xmake detects the GitHub Windows runner as `mingw`, so guard both names.
if not is_plat("macosx", "windows", "mingw") then
    add_requires("openssl")
    add_requires("cpp-httplib", {configs = {ssl = true}})
    add_defines("CPPHTTPLIB_OPENSSL_SUPPORT")
end

option("with_llama")
    set_default(true)
    set_showmenu(true)
    set_description("Enable llama.cpp integration (--with_llama=y/n, default true)")
option_end()

option("asm_hot_paths")
    set_default(false)
    set_showmenu(true)
    set_description("Enable ASM hot paths (--asm_hot_paths=y/n, default false)")
option_end()

target("traveler")
    set_kind("binary")
    add_includedirs("src")
    add_files("src/main.cpp")
    add_files("src/core/dispatcher.cpp")
    add_files("src/command/parser.cpp")
    add_files("src/tui/layout.cpp")
    add_files("src/roles/parser.cpp")
    add_files("src/roles/registry.cpp")
    add_files("src/roles/dispatch.cpp")
    add_files("src/roles/reload_cmd.cpp")
    add_files("src/safety/canonicalize.cpp")
    add_files("src/safety/pipeline.cpp")
    add_files("src/safety/event.cpp")
    add_files("src/safety/queue.cpp")
    add_files("src/safety/redact.cpp")
    add_files("src/cockpit/state.cpp")
    add_files("src/cockpit/mount.cpp")
    add_files("src/cockpit/snapshot.cpp")
    add_files("src/cockpit/strip.cpp")
    add_files("src/cockpit/bg_stream.cpp")
    add_files("src/cockpit/budget_check.cpp")
    add_files("src/cockpit/offline_mode.cpp")
    add_files("src/util/hash.cpp")
    add_files("src/editor/editor.cpp")
    add_files("src/editor/treesitter.cpp")
    add_files("src/pane/model.cpp")
    add_files("src/git/backend.cpp")
    add_files("src/llm/mode.cpp")
    add_files("src/tui/surface_model.cpp")
    add_files("src/command/registry.cpp")
    add_files("src/command/palette.cpp")
    add_files("src/command/optional_parsers.cpp")
    add_files("src/auth/credentials.cpp")
    add_files("src/auth/migration.cpp")
    add_files("src/llm/session.cpp")
    add_files("src/llm/ayane_download.cpp")
    add_files("src/persist/sessions_db.cpp")
    if not is_plat("macosx", "windows", "mingw") then
        add_files("src/auth/oauth.cpp")
        add_files("src/auth/callback_server.cpp")
        add_files("src/auth/fetch_wrapper.cpp")
        add_files("src/adapters/anthropic_adapter.cpp")
        add_files("src/adapters/openai_adapter.cpp")
        add_files("src/adapters/google_adapter.cpp")
        add_files("src/adapters/llamacpp_adapter.cpp")
        add_files("src/adapters/tool_call.cpp")
    end
    add_options("with_llama", "asm_hot_paths")
    add_deps("treesitter_grammars")
    if has_config("with_llama") then
        add_defines("TRAVELER_WITH_LLAMA")
        add_includedirs("third_party/llama.cpp/include")
        add_deps("llama")
    end
    if has_config("asm_hot_paths") then
        add_defines("TRAVELER_ASM_HOT_PATHS")
    end
    add_packages("ftxui", "tl_expected", "nlohmann_json", "sqlite3", "tree-sitter")
    if not is_plat("macosx", "windows", "mingw") then
        add_packages("cpp-httplib", "openssl")
    end
    if is_plat("windows", "mingw") then
        add_syslinks("Advapi32")
    end

target("hello-ftxui")
    set_kind("binary")
    add_files("experiments/hello-ftxui/src/main.cpp")
    add_packages("ftxui")

-- ============================================================================
-- GATE-P0-4 Test Targets
-- ============================================================================

target("test_roles")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/roles_test.cpp")
    add_files("src/roles/parser.cpp")
    add_files("src/roles/registry.cpp")
    add_packages("tl_expected")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_group("test")
    add_tests("default")

target("test_safety_corpus")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_corpus_test.cpp")
    add_files("src/safety/canonicalize.cpp")
    add_files("src/safety/pipeline.cpp")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_group("test")
    add_tests("default")

target("test_safety_queue")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_queue_test.cpp")
    add_files("src/safety/queue.cpp")
    add_files("src/safety/event.cpp")
    add_files("src/safety/canonicalize.cpp")
    set_group("test")
    add_tests("default")

target("test_safety_redact")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_redact_test.cpp")
    add_files("src/safety/redact.cpp")
    add_files("src/safety/canonicalize.cpp")
    add_files("src/safety/queue.cpp")
    add_files("src/safety/event.cpp")
    set_group("test")
    add_tests("default")

-- ============================================================================
-- WAVE-A: New Test Targets (A2 Adapters, A4 Safety, A5 Cockpit)
-- ============================================================================

-- PC-13: Adapter corpus test (Linux-only — adapter .cpp files use cpp-httplib/OpenSSL)
if not is_plat("macosx", "windows", "mingw") then
target("test_adapters_corpus")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/adapters_corpus_test.cpp")
    add_files("src/adapters/anthropic_adapter.cpp")
    add_files("src/adapters/openai_adapter.cpp")
    add_files("src/adapters/google_adapter.cpp")
    add_files("src/adapters/llamacpp_adapter.cpp")
    add_packages("tl_expected", "nlohmann_json", "cpp-httplib", "openssl")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_group("test")
    add_tests("default")
end  -- Linux-only adapter guard

target("test_safety_backtrack")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_backtrack_test.cpp")
    add_files("src/safety/canonicalize.cpp")
    add_files("src/safety/pipeline.cpp")
    set_group("test")
    add_tests("default")

target("test_safety_jsonl")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_jsonl_test.cpp")
    add_files("src/safety/event.cpp")
    set_group("test")
    add_tests("default")

target("test_safety_benchmark")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_benchmark_test.cpp")
    add_files("src/safety/canonicalize.cpp")
    add_files("src/safety/pipeline.cpp")
    set_group("test")
    add_tests("default")

target("test_safety_budget")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/safety_budget_test.cpp")
    add_files("src/cockpit/budget_check.cpp")
    add_files("src/cockpit/snapshot.cpp")
    add_files("src/safety/event.cpp")
    set_group("test")
    add_tests("default")

target("test_cockpit")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/cockpit_test.cpp")
    add_files("src/cockpit/state.cpp")
    add_files("src/cockpit/mount.cpp")
    add_files("src/cockpit/snapshot.cpp")
    add_files("src/cockpit/budget_check.cpp")
    add_files("src/cockpit/bg_stream.cpp")
    add_files("src/safety/event.cpp")
    add_packages("tl_expected")
    set_group("test")
    add_tests("default")

-- ============================================================================
-- WAVE-B: New Test Targets (B1 Editor, B2 Pane+Command, B3 Git, B4 LLM, B5 TUI)
-- ============================================================================

target("test_editor")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/editor_test.cpp")
    add_files("src/editor/editor.cpp")
    add_files("src/editor/treesitter.cpp")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_rundir(os.projectdir())
    add_deps("treesitter_grammars")
    add_packages("tree-sitter")
    set_group("test")
    add_tests("default")

target("test_pane_command")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/pane_command_test.cpp")
    add_files("src/pane/model.cpp")
    add_files("src/command/registry.cpp")
    add_files("src/command/palette.cpp")
    add_files("src/command/optional_parsers.cpp")
    set_group("test")
    add_tests("default")

target("test_git_mode")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/git_mode_test.cpp")
    add_files("src/git/backend.cpp")
    add_packages("tl_expected")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_group("test")
    add_tests("default")

target("test_llm_mode")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/llm_mode_test.cpp")
    add_files("src/llm/mode.cpp")
    add_files("src/llm/session.cpp")
    add_packages("tl_expected")
    set_group("test")
    add_tests("default")

-- ============================================================================
-- WAVE-C C1: New Test Targets (W1 Ayane download, Offline mode, LlamaCpp adapter)
-- ============================================================================

target("test_ayane_download")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/ayane_download_test.cpp")
    add_files("src/llm/ayane_download.cpp")
    add_files("src/util/hash.cpp")
    add_packages("tl_expected")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_group("test")
    add_tests("default")

target("test_offline_mode")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/offline_mode_test.cpp")
    add_files("src/cockpit/offline_mode.cpp")
    add_files("src/cockpit/mount.cpp")
    add_files("src/cockpit/snapshot.cpp")
    add_files("src/cockpit/state.cpp")
    add_packages("tl_expected")
    set_group("test")
    add_tests("default")

-- Linux-only: llamacpp_adapter.cpp uses platform-guarded code
if not is_plat("macosx", "windows", "mingw") then
target("test_llamacpp_adapter_state")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/llamacpp_adapter_test.cpp")
    add_files("src/adapters/llamacpp_adapter.cpp")
    add_packages("tl_expected", "nlohmann_json")
    set_group("test")
    add_tests("default")
end

target("test_tui_surface")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/tui_surface_test.cpp")
    add_files("src/tui/surface_model.cpp")
    add_files("src/command/parser.cpp")
    add_files("src/core/dispatcher.cpp")
    set_group("test")
    add_tests("default")

-- ============================================================================
-- PC-6: Tree-sitter parse/classification evidence (10 languages)
-- ============================================================================

target("test_treesitter")
    set_kind("binary")
    add_includedirs("src")
    add_files("tests/unit/treesitter_test.cpp")
    add_files("src/editor/treesitter.cpp")
    add_defines('TRAVELER_PROJECT_DIR="' .. os.projectdir() .. '"')
    set_rundir(os.projectdir())
    add_deps("treesitter_grammars")
    add_packages("tree-sitter")
    set_group("test")
    add_tests("default")

-- ============================================================================
-- P0-2 Live TUI binary — CEO hands-on evidence for PC-1 through PC-4
-- ============================================================================
-- Build with: xmake build -v traveler_live
-- Run with:  xmake run traveler_live  (or ./build/.../traveler_live)
-- Exits on Ctrl+C or Esc.

target("traveler_live")
    set_kind("binary")
    add_includedirs("src")
    add_files("src/tui/live_app.cpp")
    add_files("src/core/dispatcher.cpp")
    add_files("src/command/parser.cpp")
    add_files("src/tui/layout.cpp")
    add_files("src/tui/surface_model.cpp")
    add_options("with_llama", "asm_hot_paths")
    add_packages("ftxui")
    if is_plat("windows", "mingw") then
        add_syslinks("Advapi32")
    end
